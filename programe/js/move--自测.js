...
(function () {
    ...
    function move(obj, target, opt) {
        ...
        if (typeof opt.effect == 'number') {
            var str = ary[opt.effect % ary.length];
            ary = str.split('-');
            tmpEffect = ary.length >= 2 ? zhufengEffect[ary[0]][ary[1]] : zhufengEffect[ary[0]]
        } else if (opt.effect == 'object') {
            tmpEffect = effect.length >= 2 ? zhufengEffect[opt.effece[0]][opt.effece[1]] : zhufengEffect[opt.effece[0]];
        } else if (opt.effect == 'function') {
            opt.callback = opt.effet;
        }
        var begin = {};
        var change = {};
